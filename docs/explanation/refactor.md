...
`refactor` takes a crosswalk file (written by `map`, likely hand-edited
afterward) and actually renames/drops the input file's columns.
Splitting this from `map` (`docs/explanation/map.md`) is what gives
...
Run `topo-tools refactor --help` for the full, always-current option
list.
...
3. **`_03_outputs`**: exports `{name}_02` to the output file. No hard
   gate: `refactor` only renames/drops columns, it never touches
   geometry.
...
if the intent is simply to retain it unchanged. `map` always
proposes retaining an unmatched column under its original name rather
than leaving it ambiguous, so an unedited crosswalk from `map`

...
`check_valid_topology()` (`core/coverage.py`) runs on the final `_05` table and raises `RuntimeError` on failure: it chains `check_invalid_edges()` (`ST_CoverageInvalidEdges_Agg(geom) IS NOT NULL`, via `has_invalid_edges`) then `check_gaps()` (via `has_gaps`). Both unnest MultiPolygon rows into single-polygon parts first, so a coverage split into multiple parts (e.g. mainland + offshore islet) doesn't hide a real interior-ring gap. There is no separate area-based check or epsilon-based warning tier; these are the only two topology gates in the pipeline.

`check_valid_topology()` defaults `gap_maximum_width` to `SNAP_TOLERANCE` (the same name GEOS's own `CoverageCleaner` uses, see `docs/adr/0002`), tolerating a wider hole: `edge-match`/`edge-mosaic`/`topo-clean`/`edge-stitch` all rely on this default, since a wider hole in their output may be a legitimate hole in the parent/clip layer's own shape (or a real gap left by design) rather than a defect (see `docs/adr/0035`, `docs/adr/0039`, `docs/explanation/edge_match.md`'s "`check_valid_topology` and parent-layer gaps"). `edge-extend` is the one exception, passing `gap_maximum_width=0` explicitly, so any interior hole of any size still raises: it has no parent/clip layer, so its own footprint is the only thing it's guaranteeing coverage of, and a leftover hole here is always a bug.

...
`extend` calls `check_valid_topology()` with its default `max_gap_width=None`, so any interior hole of any size still raises: `extend` has no parent/clip layer, so its own footprint is the only thing it's guaranteeing coverage of, and a leftover hole here is always a bug. `match`/`mosaic` pass `max_gap_width=SNAP_TOLERANCE` instead, since a wider hole in their output may be a legitimate hole in the parent/clip layer's own shape rather than a defect (see `docs/adr/0035`, `docs/explanation/match.md`'s "`check_valid_topology` and parent-layer gaps").

...
`check_valid_topology()` defaults `gap_maximum_width` to `SNAP_TOLERANCE` (the same name GEOS's own `CoverageCleaner` uses, see `docs/adr/0002`), tolerating a wider hole: `match`/`mosaic`/`clean`/`stitch` all rely on this default, since a wider hole in their output may be a legitimate hole in the parent/clip layer's own shape (or a real gap left by design) rather than a defect (see `docs/adr/0035`, `docs/adr/0039`, `docs/explanation/match.md`'s "`check_valid_topology` and parent-layer gaps"). `extend` is the one exception, passing `gap_maximum_width=0` explicitly, so any interior hole of any size still raises: it has no parent/clip layer, so its own footprint is the only thing it's guaranteeing coverage of, and a leftover hole here is always a bug.

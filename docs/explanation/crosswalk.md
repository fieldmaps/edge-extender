...
`crosswalk` runs `map` immediately followed by `refactor` in one call,
mirroring the `clean` (wraps `detect` + fix) pattern: the two underlying
...
```python
from topo_tools.api.crosswalk import crosswalk

crosswalk("example.parquet")
```

`TARGET_SCHEMA_FILE`, `OUTPUT_FILE`, and `CROSSWALK_FILE` are all
positional and optional, with the same defaults as standalone `map`
(crosswalk path) and `refactor` (mapped-output path).

To iterate: hand-edit the written crosswalk CSV, then re-run standalone
`refactor` on it. Re-running `crosswalk` always maps fresh from scratch,
so it never sees hand edits from a prior run.

## Pipeline

1. **`_01_inputs`**: reuses `core.map._01_inputs.main()` directly, reading
   and reprojecting into `{name}_01`.
2. **`_02_map`**: reuses `core.map._02_map.main()` directly, writing the
   crosswalk to `{name}_02` exactly as standalone `map` would.
3. **`_03_apply`** (new, `core/crosswalk/_03_apply.py`): applies `{name}_02`
   to `{name}_01` via `core.refactor`'s stages, see "Table namespacing"
   below.
4. **`_04_outputs`** (new, `core/crosswalk/_04_outputs.py`): exports the
   crosswalk CSV by reusing `core.map._03_outputs.main()`, and the mapped
   file by reusing `core.refactor._03_outputs.main()`.

## Table namespacing

`core.map._02_map.main()` and `core.refactor._02_rename.main()` each
...
memory-constrained deployment targets. `core.refactor._02_rename.main()`
...
`core.refactor._03_outputs.main()`'s own cleanup uses, raises a Catalog
...
crosswalk would apply before handing it to standalone `refactor`), then
calls `core.refactor._01_inputs.validate_and_materialize_crosswalk()`
...
fresh `map`-generated crosswalk should always satisfy it by construction,
so a failure here would mean a bug in `map` itself, caught immediately
instead of silently propagating into a wrong renamed output.

## Not modeled in v1

- `crosswalk` cannot resume from a hand-edited crosswalk; it always maps
  fresh. The iteration workflow (hand-edit, then re-apply) goes through
  standalone `refactor`, not `crosswalk`, by design (see
  `docs/reference/crosswalk.md`).

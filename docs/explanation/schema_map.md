...
contains that parent's value. `schema-map` never uses column name or value
...
1. **Group every column, code or name alike, by cardinality and
   bijection** (`_build_level_groups()`, `core/schema_map/_02_map.py`): no
...
5. **Number and emit rows**: a chain position's level number is always
   ...
   drops ADR-0057's position-based admin0 exclusion: `schema-map` has no country
...
- Reusing this matching logic from inside `edge-match`/`edge-mosaic`/`topo-clean` isn't
  wired up; nothing currently needs it internally. If that changes, follow
  the precedent `docs/adr/0028` sets for `topo-detect`: extract the shared

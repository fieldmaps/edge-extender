...
`detect` scans a single polygon layer for gap/overlap coverage defects and
reports them, without fixing anything. Extracted out of `clean`'s own
issue-detection stage (`core/clean/_02_issues.py`, moved verbatim to
`core/detect/_02_issues.py`) so it can be run standalone, the same way
`assign`/`clip`/`stitch` were extracted as reusable primitives out of
`match`. `clean` still detects defects as its first real step, it just
calls `core.detect`'s stage function directly instead of owning the logic
itself, the same pattern `match`/`mosaic` use for `core.assign`/
`core.clip`/`core.stitch`, and `match`/`change` use for `core.extend`'s.
See `docs/adr/0028`.
...
Run `topo-tools detect --help` for the full, always-current option list.

## Pipeline

1. **`_01_inputs`**: reads and reprojects to EPSG:4326 via the shared
   `core.io.read_and_reproject()` helper, **without** an auto-clean
   pre-check. This is deliberate: `detect`'s whole purpose is to report
...
3. **`_03_outputs`**: exports `{name}_02` to the issues file. No hard
   gate: unlike every other tool, `detect` never modifies geometry, so
...
Earlier versions also detected (but `clean` never auto-fixed) slivers,
...
is always Polygon, so any of `extend`'s four export formats (including
Shapefile) can hold the issues file. `clean`'s own issues output extends
this schema further with measured fix outcomes (`fixed`,
`filled_area_m2`, `unit_a_area_change_m2`, `unit_b_area_change_m2`), see
`docs/explanation/clean.md`; `detect`'s own output never has those
...
for a reporting column. `detect` reports every detected gap and overlap
...
before sliver detection/reporting was removed from what was then `clean`'s
...
`detect` has no resampling knob to fall back on (nor does `extend`
...
detection out of `clean` into its own tool. See `docs/adr/0028`.

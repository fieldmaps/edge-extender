...
2. **`_02_issues.main`** — Detects gap/overlap defects, writing one issues
   ...
   `docs/clean.md`.
3. **`_03_clean.main`** — Fixes gaps/overlaps via a single call to the
   shared `core.coverage.coverage_clean()` (gated: a no-op copy only if the
   input has no coverage violations *and* no detected gap qualifies to fill
   under the resolved `gap_maximum_width` — `has_coverage_violations()`
   alone can't stand in for "nothing to fix," since it never detects
   gaps), writing `{name}_03`. Raises immediately if the result still has
   invalid edges, fails a total-area sanity floor, has a fid that eroded
   with no detected defect of its own to explain it, or has a fid whose
   fixed geometry isn't a Polygon/MultiPolygon. Logs the accepted result's
   total area change (gained/lost, as a percentage) either way. See
   `docs/clean.md`.
...
- **`ST_CoverageClean`'s `gap_maximum_width` has no GEOS-native auto-fill default.** Verified against upstream source (duckdb-spatial's `geos_module.cpp`, GEOS's `CoverageCleaner.h`/`.cpp`): the C++ class member is hardcoded to `0.0`, and a negative/omitted value is a no-op that leaves it there — unlike `snapping_distance`, which does have a real computed auto-default (`extent_diameter / 1e8`). `clean`'s `--gap-width auto` mode computes an explicit width from the widest *thin* detected gap; `all` mode uses a fixed `GAP_MAXIMUM_WIDTH_ALL_DEG = 360` sentinel, confirmed safe across the full `0`–`360°` range on real defect data (see next bullet). See `docs/clean.md`.
- **`coverage_clean()` (`core/coverage.py`) must call `ST_CoverageClean` positionally, never via DuckDB's `:=` named-argument syntax.** DuckDB binds named arguments to compiled/extension scalar functions purely by position, silently discarding the name — filed upstream as duckdb/duckdb#24574. The previous conditional named-arg construction (appending `snapping_distance := ...` only when set) shifted a caller-supplied `gap_maximum_width` into the `snapping_distance` slot whenever `snapping_distance` was left at `auto`/`None`, which is `clean`'s default. That bug was misdiagnosed for a full investigation as two independent `ST_CoverageClean` instabilities — narrow, non-monotonic invalid-edges/`TopologyException` at specific widths, and silent area erosion at wide widths — and worked around with `_03_clean.py`'s `GAP_WIDTH_ESCALATION_FACTORS` retry ladder. Both symptoms vanished once `coverage_clean()` switched to a fully positional call (`geoms, snapping_distance, gap_maximum_width`, `-1` for an omitted value): confirmed by sweeping `gap_maximum_width` from `1e-6` to `360°` against the real 164-fid/190km² admin-boundary layer that originally exhibited both symptoms — flat, monotonic area, zero invalid edges throughout. The escalation ladder was removed; `_03_clean.py` now makes a single `ST_CoverageClean` call, validates it against `has_coverage_violations()`, a total-area sanity floor (anchored to detected overlap area, not a flat fraction of the whole dataset -- `AREA_NOISE_FACTOR`/`OVERLAP_LOSS_HEADROOM`), a per-fid erosion check, and a geometry-type check, and raises immediately if any fails. See `docs/clean.md`.

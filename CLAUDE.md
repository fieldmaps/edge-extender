...
- **`match`/`mosaic`/`stitch` call `check_valid_topology()` with `max_gap_width=SNAP_TOLERANCE`, not the strict default `extend` uses**: a wider leftover gap may be a real hole in the parent/clip layer's own shape (e.g. Lesotho inside South Africa), not a defect, so it's reported as a `kind='gap'` row in the issues report instead of raising (see `docs/adr/0035`, `docs/adr/0038`). `clean`/`match`/`mosaic`/`stitch` now share one issues-table column schema and skip writing the file entirely when it would be empty (see `docs/adr/0035`, `docs/adr/0036`).

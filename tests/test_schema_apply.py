"""Portability smoke tests for the standalone schema_apply() tool."""
...
def test_cli_help():
    result = CliRunner().invoke(cli, ["schema-apply", "--help"])
...
def test_full_run_renames_and_drops(apply_input, full_crosswalk, tmp_path):
    ...
    schema_apply(apply_input, full_crosswalk, out, overwrite=True)
...
def test_default_output_path(apply_input, full_crosswalk):
    schema_apply(apply_input, full_crosswalk, overwrite=True)
...
def test_crosswalk_missing_column_raises(apply_input, tmp_path):
    """The crosswalk never decided about "Extra": schema-apply must not guess."""
    ...
    with pytest.raises(ValueError, match="not decided in the crosswalk"):
        schema_apply(apply_input, crosswalk, tmp_path / "out.parquet", overwrite=True)
...
def test_crosswalk_not_a_list_raises(apply_input, tmp_path):
    crosswalk = tmp_path / "crosswalk.json"
    crosswalk.write_text(json.dumps({"Name_Old": "adm1_name"}))
    with pytest.raises(ValueError, match="must be a JSON list"):
        schema_apply(apply_input, crosswalk, tmp_path / "out.parquet", overwrite=True)


def test_crosswalk_entry_missing_source_column_raises(apply_input, tmp_path):
    crosswalk = tmp_path / "crosswalk.json"
    crosswalk.write_text(json.dumps([{"target_column": "adm1_name"}]))
    with pytest.raises(ValueError, match="source_column key"):
        schema_apply(apply_input, crosswalk, tmp_path / "out.parquet", overwrite=True)
...
def test_target_column_with_quote_round_trips(apply_input, tmp_path):
    ...
    out = tmp_path / "mapped.parquet"
    schema_apply(apply_input, crosswalk, out, overwrite=True)
...
def test_cli_error_on_existing_output(apply_input, full_crosswalk, tmp_path):
    ...
    result = CliRunner().invoke(
        cli, ["schema-apply", str(apply_input), str(full_crosswalk), str(out)]
    )
    assert result.exit_code != 0
    assert "output already exists" in result.output


def test_schema_apply_steps(apply_input, full_crosswalk, tmp_path):

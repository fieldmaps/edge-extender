"""Public API: map, then immediately apply, a source-column -> target crosswalk."""

from logging import getLogger
from pathlib import Path

from topo_tools.core.crosswalk import _03_apply as apply_stage
from topo_tools.core.crosswalk import _04_outputs as outputs
...
from topo_tools.core.map import _01_inputs as inputs
from topo_tools.core.map import _02_map as map_stage
from topo_tools.core.map._target_schema import (
...
def crosswalk(  # noqa: PLR0913
    ...
    *,
    ...
    """Map a crosswalk, then immediately apply it (map + refactor in one call).

    Iterate by hand-editing the written crosswalk CSV and re-running
    standalone `refactor` on it, not by re-running `crosswalk`.
    """
    ...
    check_overwrite(crosswalk_path, overwrite=overwrite)

    name = input_basename(input_path).replace(".", "_") + "_crosswalk"

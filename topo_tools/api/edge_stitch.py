...
from topo_tools.core.edge_stitch import _01_inputs as inputs
from topo_tools.core.edge_stitch import _02_clean as clean
from topo_tools.core.edge_stitch import _03_outputs as outputs
...
def stitch(  # noqa: C901, PLR0913
    ...
    *,
    ...
    # "_edge_stitch" keeps every table/file this call creates distinct from
    # another tool's run against the same input_path/tmp_dir.
    name = (
        input_basename(single_path).replace(".", "_") + "_edge_stitch"
        if single_path is not None
        else output_path.name.replace(".", "_") + "_edge_stitch"
    )

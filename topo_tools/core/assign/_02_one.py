...
def _build_pairs(conn: DuckDBPyConnection, name: str) -> None:
    ...
    # Oversized parent parts: tile the boundary, join children to tiles by
    # bbox, sum area across tiles instead of one intersection against the
    # whole (possibly multi-million-vertex) part.
    heavy_parts = conn.execute(f"""--sql
        SELECT part_id, fid FROM "{name}_02_tmp2"
        WHERE n_points >= {CLIP_TILE_MIN_VERTICES}
    """).fetchall()
    for part_id, parent_fid in heavy_parts:
        conn.execute(f"""--sql
            CREATE OR REPLACE TABLE "{name}_02_heavy_src" AS
            SELECT part_geom AS geom FROM "{name}_02_tmp2" WHERE part_id = {part_id}
        """)
        subdivide_boundary(
            conn, f"{name}_02_heavy_src", "geom", f"{name}_02_heavy_tiles_raw"
        )
        conn.execute(f"""--sql
            CREATE OR REPLACE TABLE "{name}_02_heavy_tiles" AS
            SELECT geom, {bbox_columns_sql("geom")} FROM "{name}_02_heavy_tiles_raw"
        """)
        conn.execute(f"""--sql
            INSERT INTO "{name}_02_pairs_raw"
            SELECT c.fid AS child_fid, {parent_fid} AS parent_fid,
                   SUM(ST_Area(ST_Transform(
                       ST_Intersection(c.part_geom, t.geom),
                       'EPSG:4326', '{EQUAL_AREA_CRS}'
                   ))) AS shared_area
            FROM "{name}_02_tmp1" c
            JOIN "{name}_02_heavy_tiles" t
